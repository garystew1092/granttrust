...
const selectedRole= ({target})=>{
  if (target.value ==="member"){
    setRole("member")
...
if (role ==="admin"){
    ...
    //Check if authorized to be an administrator
    if (clientCode!=="4588944" ){
      setError("Admin Code is wrong")
      return;
    }
}else{
  setClientCode("")
      ...
      <div className="w-full max-w-md border border-gray-200 rounded-lg shadow-sm p-6 space-y-6 bg-white">
        <form onSubmit={handleForm}>
          ...
          <div className="space-y-4">
            ...
            {showClientCode && (
              <div>
                ...
                <input
                  ...
                  type="text"
                  value={clientCode}
                  onChange={({target})=> setClientCode(target.value)}
                  id="admincode"
                  className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent placeholder-gray-400"
                />
              </div>
            ...
            {error && <div id ="error">
              <small className="text-red-500 text-[10px] font-semibold">{error}</small>
            </div>
          ...
          <button
            type="submit"
            className=" curosr-pointer w-full flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-600 text-white font-medium py-2 px-4 rounded-md transition-colors duration-200"
          >
